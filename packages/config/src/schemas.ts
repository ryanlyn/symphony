...
import { DEFAULT_TRACKER_KIND } from "./defaults.js";
...
interface FlatShapeDeprecationMeta {
  ...
  /** Effective bundle name when the selector is omitted. */
...
export const trackerRecordSchema = z
  ...
  .meta({
    flatShape: {
      ...
      bundleNameKey: "kind",
      defaultBundleName: DEFAULT_TRACKER_KIND,
...
function collectFlatShape(
  ...
  const bundle = suggestedBundleName(rawSection[flat.bundleNameKey], flat.defaultBundleName);
  for (const key of Object.keys(rawSection)) {
...
function suggestedBundleName(kind: unknown, defaultBundleName: string): string {
  return typeof kind === "string" && kind.trim() !== "" ? kind.trim() : defaultBundleName;
}
