...
test("SSH target parsing and command args match host:port behavior", () => {
  ...
  assert.deepEqual(sshArgs("localhost:2222", "echo ready", process.env), [
    "-T",
    "-p",
    "2222",
    "--",
  ...
  assert.deepEqual(reverseTunnelArgs("localhost:2222", 9000, "127.0.0.1", 4040, process.env), [
    "-T",
    "-N",
    "-o",
...
test("SSH args reject empty and option-like targets", () => {
  ...
  for (const target of unsafeTargets) {
    assert.throws(() => sshArgs(target, "echo ready", process.env), /invalid_ssh_destination/);
    assert.throws(
      () => reverseTunnelArgs(target, 9000, "127.0.0.1", 4040, process.env),
...
`,
  ...
  await assert.rejects(
    () => runSsh("localhost", "printf ok", { env: process.env }),
    /ssh_not_found/,
  );
...
`,
  ...
  await assert.rejects(
    () => runSsh("localhost", "printf ok", { env: process.env, sshExecutablePath, timeoutMs: 20 }),
...
`,
  ...
  await assert.rejects(
    () =>
      runSsh("localhost", "printf ok", { env: process.env, sshExecutablePath, timeoutMs: 1000 }),
...
`,
  ...
  await writeRemoteFile("localhost", remotePath, payload, {
    env: process.env,
    mode: 0o755,
    sshExecutablePath,
  });
