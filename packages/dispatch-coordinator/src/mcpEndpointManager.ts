// The CONCRETE per-run McpEndpointManager.
//
...
// so two co-resident runs on one machine each carry their own per-run claim.
//
// Dependency direction: this package must stay free of any `@lorenz/mcp` / tunnel
// RUNTIME dependency, so the concrete acquire function
