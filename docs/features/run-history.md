...
It reads the runtime's in-memory run history: a bounded ring buffer of the **last 50** completed runs, maintained by `ProjectionActor` (`recordRunHistory`, cap 50, newest first). There is no database. On restart the ring buffer starts empty and refills as new runs finish. History past the last 50 runs is gone, so capture the output you need before it ages out.
