...
| --- | --- |
| Claude Code executor | Adds `agent.kind: "claude"` support, including Claude CLI execution, JSONL event parsing, generated MCP config, remote worker support, and Claude-specific runtime settings. |
...
| Claude parity and MCP handling | Brings Claude turn handling closer to Codex with consistent issue context injection, matching inactivity timeout semantics, safer MCP config generation, and better remote cleanup behavior. |
