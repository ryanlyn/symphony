...
- `lorenz-pull`: use this when push is rejected or sync is not clean (non-fast-forward,
  merge conflict risk, or stale branch).

## Steps

1. Identify current branch and confirm remote state.
2. Run the project's pre-push validation before pushing. Discover the
   command from `AGENTS.md`, `CLAUDE.md`, `CONTRIBUTING.md`,
   `CONTRIBUTORS.md`, `.agents/README.md`, or a repo-local skill. If
   no project-specific validation is documented, skip this step and
   note it in the PR.
...
# Run the repo's pre-push validation command (discover it from
# AGENTS.md / CLAUDE.md / CONTRIBUTING.md / CONTRIBUTORS.md /
# .agents/README.md / repo-local skills).
...
# Write/edit PR body to match the inlined template above.
# Example workflow:
...
- Distinguish sync problems from remote auth/permission problems:
  - Use the `lorenz-pull` skill for non-fast-forward or stale-branch issues.
