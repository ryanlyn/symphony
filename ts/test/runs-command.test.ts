...
import { parseRunsArgs, runRunsCommand } from "../src/runsCommand.js";
...
test("runs command queries the observability API and renders a run table", async () => {
  ...
  try {
    ...
    const output = await runRunsCommand([
      "--url",
...
test("runs command reports Elixir-shaped 404 and 503 errors", async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => runRunsCommand(["--url", baseUrl, "--id", "missing"]),
      /Run not found/,
    );
    await assert.rejects(() => runRunsCommand(["--url", baseUrl]), /Snapshot timed out/);
  } finally {
    await new Promise<void>((resolve, reject) =>
      server.close((error) => (error ? reject(error) : resolve())),
    );
...
test("runs command uses workflow-derived default server host and port", async () => {
  ...
  try {
    ...
    const output = await runRunsCommand(["--limit", "1"]);
    assert.match(output, /Run History/);
  } finally {
