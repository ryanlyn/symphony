...
import { SymphonyRuntime } from "../src/runtime.js";
import {
...
import type { Issue, RunResult, WorkflowDefinition } from "../src/index.js";
import { initGitRepo, tempDir, writeExecutable } from "./helpers.js";
...
test("runtime once claims an eligible issue, applies updates, and records completion", async () => {
  const issue = issueFixture("issue-1", "MT-1");
  const doneIssue = { ...issue, state: "Done", stateType: "completed" };
  const runtime = new SymphonyRuntime({
    workflow: workflowFixture(),
    client: {
      fetchCandidateIssues: async () => [issue],
  ...
  const runtime = new SymphonyRuntime({
    ...
    client: {
      ...
      fetchIssuesByIds: async (ids) => {
        if (ids.includes(staleIssue.id))
          return [{ ...staleIssue, state: "Done", stateType: "completed" }];
        return [];
      },
    },
    runner: async () => {
...
test("runtime reconciliation removes terminal retry workspaces before polling", async () => {
  ...
  const doneIssue = { ...activeIssue, state: "Done", stateType: "completed" };
