...
import { issuePayload, runsPayload, statePayload } from "../src/index.js";
import type { RuntimeSnapshot } from "../src/runtime.js";
...
test("presenter humanizes structured agent messages at the JSON API boundary", () => {
  ...
  snapshot.runHistory[0]!.lastEvent = "agent_message_delta";
...
test("presenter does not depend on the Ink TUI module for API projections", () => {
  const source = fs.readFileSync(
    path.join(import.meta.dirname, "..", "src", "presenter.ts"),
    "utf8",
  );
  assert.doesNotMatch(source, /from "\.\/tui\.js"/);
});
