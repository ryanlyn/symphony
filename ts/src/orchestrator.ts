...
import { retryBackoffMs } from "./policies/retryPolicy.js";
import { mergeMonotonicUsage } from "./policies/usagePolicy.js";
import type {
  AgentUpdate,
  DispatchBlockEntry,
  Issue,
...
export class Orchestrator {
  ...
  eligibleIssues(issues: Issue[]): Issue[] {
    ...
    return sortForDispatch(issues).filter((issue) => {
      ...
      if (retry && retry.dueAt.getTime() > Date.now()) return false;
  ...
  claim(issue: Issue): RunningEntry | null {
    const retry = this.state.retryAttempts.get(issue.id);
    if (retry && retry.dueAt.getTime() <= Date.now()) this.releaseStaleClaimsForRetry(issue.id);
    const runningByState = new Map<string, number>();
    for (const entry of this.state.running.values()) {
      runningByState.set(entry.issue.state, (runningByState.get(entry.issue.state) ?? 0) + 1);
    }
  ...
  private selectWorkerHost(): string | null | undefined {
    const hosts = this.settings.worker.sshHosts;
    ...
    for (const host of hosts) {
      ...
      if (count < cap && count < selectedCount) {
        selected = host;
        selectedCount = count;
      }
    }
  ...
  applyUpdate(issueId: string, slotIndex: number, update: AgentUpdate): void {
    ...
    entry.lastAgentTimestamp = update.timestamp ?? new Date();
  ...
  finish(
    ...
    if (normal) {
      ...
      this.state.retryAttempts.set(issueId, {
        ...
        attempt,
        dueAt: new Date(
          Date.now() + retryBackoffMs(attempt, this.settings.agent.maxRetryBackoffMs, retryKind),
        ),
        slotIndex,
