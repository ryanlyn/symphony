import { issueHasOpenBlockers, issueIsActive, routedToThisWorker, slotKey } from "./dispatch.js";
import { Orchestrator } from "./orchestrator.js";
import { settingsForIssueState, validateDispatchConfig } from "./config.js";
import { createTrackerClient } from "./trackerClient.js";
import { runAgentAttempt, type RunResult } from "./agentRunner.js";
import { deleteResumeState } from "./resumeState.js";
import { removeIssueWorkspaces } from "./workspace.js";
import { appendLogEvent } from "./logFile.js";
import { ProjectionActor } from "./projections/runtimeProjection.js";
import { RetryScheduler } from "./runtime/retryScheduler.js";
import type {
  DispatchBlockEntry,
  Issue,
  RunningEntry,
...
export interface RuntimeRunHistoryEntry {
  ...
  agentKind: string;
  outcome: "success" | "failed" | "stalled" | "canceled";
  turnCount: number;
...
export class SymphonyRuntime {
  ...
  constructor(private readonly input: SymphonyRuntimeOptions) {
    this.client = input.client ?? createTrackerClient(input.workflow.settings);
  ...
  private async reloadWorkflowIfConfigured(): Promise<void> {
    ...
    try {
      ...
      if (!this.input.client) this.client = createTrackerClient(workflow.settings);
  ...
  private async reconcileTrackedIssues(): Promise<void> {
    ...
    for (const issue of refreshed) {
      ...
      if (isTerminalIssue(issue, this.workflow.settings)) {
        await removeIssueWorkspaces(
  ...
  private async cleanupTerminalWorkspacesOnce(): Promise<void> {
    ...
    try {
      ...
      for (const issue of terminalIssues) {
        await removeIssueWorkspaces(this.workflow.settings, issue.identifier);
        cleaned += 1;
      }
  ...
  private addEvent(type: string, message: string): void {
    ...
    void appendLogEvent(this.workflow.settings.logging.logFile, {
      at: event.at,
      event: type,
      message,
    });
...
function reconciliationStopReason(issue: Issue, settings: WorkflowDefinition["settings"]): string {
...
function isTerminalIssue(issue: Issue, settings: WorkflowDefinition["settings"]): boolean {
  const normalized = issue.state.trim().toLowerCase();
  return settings.tracker.terminalStates.some(
