...
import { humanizeAgentMessage } from "./agentMessages.js";
...
export function RuntimeDashboard({
  ...
  return (
    <Box flexDirection="column" marginTop={1}>
      <Text>
        {formatElixirStyleDashboard(snapshot, { dashboardUrl, projectUrl, throughputTps })}
      </Text>
    </Box>
  );
}
...
export function formatElixirStyleDashboard(
  ...
  lines.push(
    `${b("│ Next refresh: ", ansi)}${s("90", formatNextRefresh(snapshot.poll.nextPollAt, now), ansi)}`,
  );
...
function formatRunningRow(
  ...
  const color = rowColor(run, stage, event);
...
function formatDispatchBlockRow(block: unknown, ansi: boolean): string {
  ...
  const state = stringAt(block, ["state"]);
  const suffix = state ? ` state=${sanitize(state)}` : "";
  return `│  ${s("33", "!", ansi)} ${s("31", sanitize(identifier), ansi)} ${s("2", `reason=${sanitize(reason)}${suffix}`, ansi)}`;
}
...
function rowColor(run: RuntimeSnapshot["running"][number], stage: string, event: string): string {
  if (stage === "retrying") return "34";
  if (run.agentKind === "claude") return "32";
  if (event.startsWith("thread token usage")) return "33";
  return "35";
}
