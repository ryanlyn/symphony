import { assert } from "@symphony/test-utils";
import { test } from "vitest";
import type { BoxPoolSettings } from "@symphony/domain";
import { systemClock, withDerivedMaxInFlight } from "@symphony/domain";

// Everything under test is pulled from the public barrel so this file exercises
// the package exactly as a downstream consumer (the runtime/CLI) would.
import * as barrel from "../src/index.js";
import {
  BoxDriverRegistry,
  createBoxPool,
  createMutex,
  defaultBoxDriverRegistry,
  FakeBoxDriver,
  POOL_OWNED_LABEL,
  registerFakeBoxDriver,
} from "../src/index.js";

// A full settings object so `createBoxPool` has every knob it reads; only
// `enabled`/`driver` matter for these wiring tests but the type wants the rest.
function poolSettings(overrides: Partial<BoxPoolSettings> = {}): BoxPoolSettings {
  const { maxInFlight, slotsPerMachine, ...rest } = overrides;
  return withDerivedMaxInFlight({
    enabled: true,
    driver: "fake",
    min: 0,
    max: 1,
    warm: 0,
    slotsPerMachine: slotsPerMachine ?? maxInFlight ?? 1,
    ttlMs: 3_600_000,
    idleReapMs: 300_000,
    acquireTimeoutMs: 1_000,
    reapIntervalMs: 15_000,
    staleHeartbeatMs: 600_000,
    drainDeadlineMs: 30_000,
    ...rest,
  });
}

const baseDeps = {
  clock: systemClock,
  logEvent: () => undefined,
};

test("the barrel has NO import side effects: nothing is registered into the default registry", () => {
  // Importing the barrel must register nothing: drivers live in extensions (and
  // the fake in the SDK), and only the composition root wires them in. This
  // test runs FIRST in this file, before any test below touches the default
  // registry, so the assertion observes the pure import-time state.
  assert.deepEqual(defaultBoxDriverRegistry.kinds(), []);
  assert.equal(defaultBoxDriverRegistry.get("fake"), undefined);
});

test("the barrel exposes exactly the pool engine surface plus the SDK re-exports (no conformance)", () => {
  // Pin the runtime (value) export list so a side-door export - including the
  // SDK conformance kit, which must stay behind `@symphony/box-sdk/conformance` -
  // cannot creep into the engine barrel unnoticed.
  assert.deepEqual(Object.keys(barrel).sort(), [
    "BoxDriverRegistry",
    "FakeBoxDriver",
    "POOL_OWNED_LABEL",
    "createBoxPool",
    "createMutex",
    "defaultBoxDriverRegistry",
    "registerFakeBoxDriver",
  ]);
});

test("the barrel re-exports the SDK driver surface as usable values", () => {
  assert.equal(typeof BoxDriverRegistry, "function");
  assert.equal(typeof FakeBoxDriver, "function");
  assert.equal(typeof registerFakeBoxDriver, "function");
  assert.equal(typeof createMutex, "function");
  assert.equal(POOL_OWNED_LABEL, "symphony.pool=worker-box-pool");
  assert.ok(defaultBoxDriverRegistry instanceof BoxDriverRegistry);
});

test("createBoxPool resolves the driver from an explicit registry passed via deps.drivers", () => {
  const drivers = new BoxDriverRegistry();
  registerFakeBoxDriver({ boxDrivers: drivers });

  const pool = createBoxPool(poolSettings(), { ...baseDeps, drivers });

  // A live pool exposes the public surface and can accept work immediately.
  assert.ok(typeof pool.acquire === "function");
  assert.equal(pool.canAcquire(), true);

  const snapshot = pool.snapshot();
  assert.equal(snapshot.enabled, true);
  assert.equal(snapshot.driver, "fake");
});

test("createBoxPool fails loud with box_pool_driver_unavailable for an unregistered kind, listing the known kinds", () => {
  const drivers = new BoxDriverRegistry();
  registerFakeBoxDriver({ boxDrivers: drivers });

  assert.throws(
    () => createBoxPool(poolSettings({ driver: "static-ssh" }), { ...baseDeps, drivers }),
    /box_pool_driver_unavailable: static-ssh \(known kinds: fake\)/,
  );
});

test("createBoxPool falls back to the process-wide default registry when deps.drivers is absent", () => {
  // An empty default registry fails loud at construction (the daemon fails fast)...
  assert.throws(() => createBoxPool(poolSettings(), baseDeps), /box_pool_driver_unavailable: fake/);

  // ...and once the composition root registers a driver into the default
  // registry, the same no-registry construction succeeds. This test pins the
  // defaultBoxDriverRegistry fallback and therefore mutates the process-wide
  // registry; it runs LAST so the import-side-effect test above stays pure.
  registerFakeBoxDriver();
  const pool = createBoxPool(poolSettings(), baseDeps);
  assert.equal(pool.snapshot().driver, "fake");
});
