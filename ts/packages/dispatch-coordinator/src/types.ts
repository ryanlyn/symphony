...
// only cross-package value dependencies are the canonical box-pool/domain nouns.
//
...
// hono, etc.) - keeping this package (and `worker-box-pool`) free of any
...
import type { BoxOutcome } from "@symphony/worker-box-pool";

/**
 * One run's slot over a leased machine. A `RunSlot` fuses the machine-side
 * {@link BoxLease} sub-handle (settle/heartbeat) with this run's optional
...
export interface RunSlot {
  ...
  /** The wrapped `BoxLease` generation id; rotates on retry, settle-guarded. */
  ...
  /** Mirrors `BoxLease.acquiredAtMs` (the lease-bind timestamp). */
  readonly acquiredAtMs: number;
  /** Forwards to `BoxLease.heartbeat()` (orphan-detection keepalive). */
  heartbeat(): void;
  /**
   * Settle this slot exactly once: close the endpoint (noop for null) THEN
   * settle the wrapped lease with `outcome` (`healthy` keeps the box, `poison`
...
/**
 * Request to acquire a {@link RunSlot}. Mirrors the box-pool `AcquireRequest`
...
export interface AcquireRunSlotRequest {
   ...
   * NOT its own `BoxPoolSettings`, which has no `server.port` and would make an
   ...
   * `mcpEndpoint` stays null), so a Codex box-pool run is never SKIPPED by
