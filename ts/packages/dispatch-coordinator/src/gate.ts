// Pure predicate behind the `worker.box_pool.slots_per_machine > 1` co-residence
...
/**
 * Returns an operator-facing error message when `boxPool` would be unsafe to run,
...
export function checkSlotsPerMachineGate(
  ...
  if (!boxPool || boxPool.enabled === false || boxPool.slotsPerMachine <= 1) return null;

  if (capabilities?.perRunEndpoint !== true) {
    return (
      "worker.box_pool.max_in_flight > 1 requires a dispatch coordinator with per-run MCP " +
      "endpoints (capabilities.perRunEndpoint), which the current build does not provide"
    );
  }
  if (boxPool.coResidence !== true) {
    return (
      "worker.box_pool.max_in_flight > 1 requires the explicit worker.box_pool.co_residence " +
      "opt-in: co-residence shares one machine across runs, so a poisoned box fails every " +
      "co-resident run on recycle. Set worker.box_pool.co_residence: true to accept this " +
      "blast radius"
    );
  }
  return null;
