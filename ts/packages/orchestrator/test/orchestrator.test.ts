...
test("orchestrator reservation carries affinityHost = retry.workerHost (retry affinity preserved)", () => {
  ...
  const reservation = claimReservation(orchestrator, issue);
  assert.equal(reservation?.affinityHost, "warm-box-7:2200");
...
test("orchestrator bindReservation mints the RunningEntry with the CONCRETE bound host", () => {
  ...
  clock.advance(5_000);
  const entry = orchestrator.bindReservation(reservation!, "leased-box-3:2200");
  assert.equal(entry?.workerHost, "leased-box-3:2200");
  ...
  assert.equal(orchestrator.bindReservation(reservation!, "leased-box-4:2200"), null);
});
...
test("orchestrator cancelReservation frees the slot with NO retry record, leaving the slot re-claimable", () => {
  ...
  assert.equal(orchestrator.bindReservation(reservation!, "leased-box-9:2200"), null);
...
test("orchestrator bindReservation is token-guarded: a cancel + re-reserve defeats a late bind (ABA)", () => {
  ...
  // successor's record.
  assert.equal(orchestrator.bindReservation(first!, "stale-box:2200"), null);
  ...
  const entry = orchestrator.bindReservation(second!, "fresh-box:2200");
  assert.equal(entry?.workerHost, "fresh-box:2200");
});
...
test("orchestrator cancelReservation restores the consumed RetryEntry without clobbering a newer one", () => {
  ...
  assert.ok(again);
  const newer = { ...consumed, attempt: 9, workerHost: "warm-box-9:2200" };
...
test("orchestrator expiry sweep cancels a past-expiry reservation and restores its retry entry", () => {
  ...
  const reservation = claimReservation(orchestrator, issue);
  assert.ok(reservation);
  // Default acquireTimeoutMs (30s, no boxPool configured): expiry = 2x + 60s grace.
  ...
  assert.equal(orchestrator.bindReservation(reservation!, "late-box:2200"), null);
});
      ...
      (maxConcurrent, ops) => {
        ...
        for (const { op, pick } of ops) {
          if (op === "claim") {
            ...
            const entry = orchestrator.bindReservation(reservation!, `box-${pick}:2200`);
            if (entry) {
