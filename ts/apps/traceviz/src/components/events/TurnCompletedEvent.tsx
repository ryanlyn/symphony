        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex items-center gap-3 text-xs text-muted">
            {event.usage && (
              <>
                <span>
                  In: <span className="text-foreground">{formatNumber(event.usage.inputTokens)}</span> tokens
                </span>
                <span>
                  Out: <span className="text-foreground">{formatNumber(event.usage.outputTokens)}</span> tokens
                </span>
              </>
            )}
            {event.durationMs != null && (
              <span>
                Duration: <span className="text-foreground">{formatDuration(event.durationMs)}</span>
              </span>
            )}
          </div>
        </div>
