...
import {
  DockerBoxDriver,
  dockerBoxDriverFactory,
  registerDockerBoxDriver,
...
function makeDriver(over?: DockerDriverOverrides): DockerBoxDriver {
  const fake = fakeDocker();
  return new DockerBoxDriver({ image: IMAGE }, makeDeps(), {
    ...fake.override,
    ...over,
  });
}
...
// `boxIds` are arbitrary pool idempotency keys; the driver derives the host
...
runDriverConformanceSuite(
  () => {
    const fake = fakeDocker();
    return new DockerBoxDriver({ image: IMAGE }, makeDeps(), fake.override);
  },
  {
    suiteName: "DockerBoxDriver (fake docker CLI)",
    boxIds: ["box-a", "box-b"],
    ...
    makeUnreachable: () => {
      const fake = fakeDocker();
      const driver = new DockerBoxDriver(
        { image: IMAGE },
        makeDeps(async () => {
...
describe("DockerBoxDriver command construction", () => {
  ...
  test("provision: a custom user from the driver options is used in workerHost", async () => {
    ...
    const driver = new DockerBoxDriver({ image: IMAGE, user: "agent" }, makeDeps(), fake.override);
    const box = await driver.provision({
      boxId: "box-u",
      labels: [LABEL_POOL],
      timeoutMs: 30_000,
  ...
  test("provision: reads sshUser (camelCase) as well as user", async () => {
    ...
    const driver = new DockerBoxDriver({ image: IMAGE, sshUser: "dev" }, makeDeps(), fake.override);
    const box = await driver.provision({
      boxId: "box-c",
      labels: [LABEL_POOL],
      timeoutMs: 30_000,
  ...
  test("provision: rejects when the image driver option is missing", async () => {
    const fake = fakeDocker();
    const driver = new DockerBoxDriver({}, makeDeps(), fake.override);
    await assert.rejects(
      () => driver.provision({ boxId: "box-a", labels: [LABEL_POOL], timeoutMs: 30_000 }),
      /docker_image_required/,
    );
  });

  test("provision: maps a non-zero `docker run` exit to a thrown driver error", async () => {
    const driver = new DockerBoxDriver({ image: IMAGE }, makeDeps(), {
  ...
  test("provision: a post-create `docker port` failure force-removes the container (no leak)", async () => {
    ...
    await assert.rejects(
      () => driver.provision({ boxId: "box-leak", labels: [LABEL_POOL], timeoutMs: 30_000 }),
      /docker_port_failed/,
    );
  ...
  test("provision is idempotent across instances via the pool label on the live daemon", async () => {
    ...
    const fake = fakeDocker({ startPort: 41_000 });
    const a = new DockerBoxDriver({ image: IMAGE }, makeDeps(), fake.override);
    const first = await a.provision({ boxId: "box-keep", labels: [LABEL_POOL], timeoutMs: 30_000 });

    const b = new DockerBoxDriver({ image: IMAGE }, makeDeps(), fake.override);
...
describe("DockerBoxDriver list/destroy/probe", () => {
  test("list: docker ps filters by the pool label and parses the box-id label + port", async () => {
    ...
    const driver = new DockerBoxDriver({ image: IMAGE }, makeDeps(), fake.override);
    await driver.provision({ boxId: "box-1", labels: [LABEL_POOL], timeoutMs: 30_000 });
    await driver.provision({ boxId: "box-2", labels: [LABEL_POOL], timeoutMs: 30_000 });
  ...
  test("destroy: docker rm -f on the driverRef (container id), idempotent on already-gone", async () => {
    ...
    assert.ok(rmCall);
    assert.equal(rmCall!.includes("-f"), true);
    // The driverRef (container id), NOT the boxId, is what `docker rm` targets.
    assert.equal(rmCall![rmCall!.length - 1], box.driverRef);

    // The container is gone from the daemon.
    assert.equal(
      (await driver.list()).some((b) => b.boxId === "box-x"),
      false,
    );
  ...
  test("registerDockerBoxDriver registers the factory idempotently", () => {
    const registry = new BoxDriverRegistry();
    registerDockerBoxDriver({ boxDrivers: registry });
    // A second registration is a no-op (the kind is already registered).
    registerDockerBoxDriver({ boxDrivers: registry });

    const factory = registry.require("docker");
    assert.equal(factory, dockerBoxDriverFactory);
...
// ---------------------------------------------------------------------------
describe("DockerBoxDriver over a PATH-shimmed fake docker binary", () => {
...
`,
  ...
  test("provision -> list -> destroy round-trips through the real docker subprocess seam", async () => {
    ...
    await driver.destroy(box, { timeoutMs: 5_000, reason: "shrink" });
    assert.equal(
      (await driver.list()).some((b) => b.boxId === "real-1"),
      false,
    );
...
describe.skipIf(!LIVE)(
  "DockerBoxDriver live conformance (SYMPHONY_TS_RUN_LIVE_DOCKER_E2E=1)",
  () => {
    runDriverConformanceSuite(
      () =>
        new DockerBoxDriver(
      ...
      {
        suiteName: "DockerBoxDriver (live docker)",
