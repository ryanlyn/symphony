...
import {
  E2BWorkerDriver,
  E2B_WORKER_POOL_LABEL,
  e2bWorkerDriverFactory,
  registerE2bWorkerDriver,
...
function provisionRequest(
  ...
  return {
    workerId,
    labels: ["symphony.worker-pool", "team=core"],
    timeoutMs: 30_000,
    ...overrides,
  };
}
...
// ---------------------------------------------------------------------------
// Shared conformance suite over the fake client. workerIds are arbitrary pool
// idempotency keys (E2B is an ephemeral cloud driver; provision is keyed on
// the pool workerId via sandbox metadata). The unreachable variant injects an
// ssh probe failure.
// ---------------------------------------------------------------------------
...
runDriverConformanceSuite(
  () => {
    ...
    const ssh = okRunSsh();
    return new E2BWorkerDriver({}, makeDeps([], ssh.fn), { client });
  },
  {
    suiteName: "E2BWorkerDriver",
    workerIds: ["worker-a", "worker-b"],
    makeUnreachable: () => {
      const client = new FakeE2BClient();
      const driver = new E2BWorkerDriver(
        {},
        // A probe that fails (e.g. sshd not up yet) gates the worker to ok:false.
...
function makeDriver(template?: string): E2BWorkerDriver {
  return new E2BWorkerDriver(template ? { image: template } : {}, makeDeps(events, ssh.fn), {
    client,
  });
}
...
test("provision starts a sandbox tagged with the worker-pool label, workerId, and request labels", async () => {
  const driver = makeDriver("ghcr.io/org/worker:latest");
  const worker = await driver.provision(provisionRequest("worker-1"));

  expect(client.createCalls).toHaveLength(1);
  const call = client.createCalls[0];
  // The base image flows through from the `image` driver option as the template.
  expect(call.template).toBe("ghcr.io/org/worker:latest");
  // Metadata carries the pool-owned marker label (so list() can re-adopt
  // survivors) and the pool's idempotency key.
  expect(call.metadata[E2B_WORKER_POOL_LABEL]).toBe("true");
  expect(call.metadata["symphony.worker-id"]).toBe("worker-1");
  // The request labels are recorded on the descriptor and the sandbox metadata.
  expect(call.metadata["symphony.labels"]).toBe("symphony.worker-pool,team=core");

  // The descriptor advertises the SSH endpoint as the workerHost.
  expect(worker.workerId).toBe("worker-1");
  expect(worker.workerHost).toBe("root@1.2.3.4:22");
  expect(worker.driverRef).toBe("sbx-1");
  expect(worker.labels).toEqual(["symphony.worker-pool", "team=core"]);
  expect(worker.metadata.sandboxId).toBe("sbx-1");
});

test("provision is idempotent on workerId: a second call re-adopts the running sandbox", async () => {
  const driver = makeDriver();
  const first = await driver.provision(provisionRequest("worker-1"));
  const second = await driver.provision(provisionRequest("worker-1"));
...
test("provision re-adopts a survivor discovered via list() (cross-instance recovery)", async () => {
  // Simulate a sandbox left running by a prior daemon: it carries the worker-pool
  // label + workerId metadata but this driver instance never created it.
  client.sandboxes.set("sbx-orphan", {
    sandboxId: "sbx-orphan",
    metadata: {
      [E2B_WORKER_POOL_LABEL]: "true",
      "symphony.worker-id": "worker-1",
    },
  });
  const driver = makeDriver();
  const worker = await driver.provision(provisionRequest("worker-1"));
  ...
  expect(worker.driverRef).toBe("sbx-orphan");
});
...
test("destroy kills the sandbox and is idempotent (already-gone is ok)", async () => {
  ...
  await driver.destroy(worker, { timeoutMs: 30_000, reason: "idle" });
...
test("list returns only pool-owned running sandboxes, mapped to descriptors", async () => {
  ...
  expect(adopted?.workerId).toBe("worker-1");
  ...
  expect(adopted?.labels).toEqual([POOL_OWNED_LABEL, "symphony.worker-pool", "team=core"]);
});
...
test("probe runs runSsh printf-ready against the workerHost with the supplied timeout", async () => {
  ...
  const health = await driver.probe(worker, { timeoutMs: 12_345 });
...
test("probe gates to ok:false (not throwing) when the SSH transport errors", async () => {
  ...
  const health = await driver.probe(worker, { timeoutMs: 1_000 });
...
test("provision maps a client create failure to a typed e2b_provision_failed error", async () => {
  ...
  await expect(driver.provision(provisionRequest("worker-1"))).rejects.toThrow(
...
test("destroy maps a non-not-found client kill failure to a typed e2b_destroy_failed error", async () => {
  ...
  await expect(driver.destroy(worker, { timeoutMs: 30_000, reason: "ttl" })).rejects.toThrow(
...
test("throws e2b_client_unavailable when no client is injected and no SDK is wired", () => {
  ...
  // silently no-op.
  expect(() => new E2BWorkerDriver({}, makeDeps())).toThrow(/e2b_client_unavailable/);
});

test("registerE2bWorkerDriver without io registers a fail-loud factory", () => {
  const registry = new WorkerDriverRegistry();
  registerE2bWorkerDriver({ workerDrivers: registry });
...
test("registerE2bWorkerDriver with io registers a working factory closing over the client", () => {
  const registry = new WorkerDriverRegistry();
  registerE2bWorkerDriver({ workerDrivers: registry }, { client: new FakeE2BClient() });

  const driver = registry.require("e2b").create({}, makeDeps());
  expect(driver.kind).toBe("e2b");
});

test("e2bWorkerDriverFactory constructs a working driver over the injected client", async () => {
  const factory = e2bWorkerDriverFactory({ client });
