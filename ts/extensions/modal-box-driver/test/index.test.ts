...
import {
  ModalBoxDriver,
  modalBoxDriverFactory,
  registerModalBoxDriver,
...
function makeDriver(transport: ModalTransport, runSsh?: SshRunner): ModalBoxDriver {
  return new ModalBoxDriver({ image: "ghcr.io/org/box:latest" }, makeDeps(runSsh), {
    transport,
  });
}
...
runDriverConformanceSuite(() => makeDriver(new FakeModalTransport()), {
  suiteName: "ModalBoxDriver (fake transport)",
  boxIds: ["box-a", "box-b"],
...
describe("ModalBoxDriver lifecycle", () => {
  ...
  test("provision creates a sandbox exposing ssh and returns the endpoint as workerHost", async () => {
    ...
    assert.equal(box.driverRef, "sb-0");
  ...
  test("provision labels the sandbox with the boxId and the pool labels", async () => {
    ...
    await driver.provision({ boxId: "box-a", labels: [POOL_LABEL], timeoutMs: 30_000 });
  ...
  test("destroy terminates the sandbox via the transport", async () => {
    ...
    // The terminate hit the sandbox id (driverRef), not the boxId.
  ...
  test("destroy is idempotent (already-terminated sandbox)", async () => {
    ...
    await driver.destroy(box, { timeoutMs: 5_000, reason: "idle" });
  ...
  test("list enumerates only sandboxes carrying the pool label", async () => {
    ...
    const refs = listed.map((box) => box.driverRef);
    ...
    const adopted = listed.find((box) => box.driverRef === "sb-0");
    assert.ok(adopted);
  ...
  test("provision maps a transport fault to a modal_provision_failed error", async () => {
    ...
    await assert.rejects(
      () => driver.provision({ boxId: "box-a", labels: [POOL_LABEL], timeoutMs: 30_000 }),
  ...
  test("destroy maps a transport fault to a modal_destroy_failed error", async () => {
    ...
    await assert.rejects(
      () => driver.destroy(box, { timeoutMs: 5_000, reason: "shrink" }),
  ...
  test("destroy terminates a survivor this instance did not provision (hydrate/reconcile)", async () => {
    ...
    const listed = await driver.list();
    const survivor = listed.find((box) => box.driverRef === "sb-survivor");
    assert.ok(survivor);
  ...
  test("registerModalBoxDriver without io registers a fail-loud factory", () => {
    const registry = new BoxDriverRegistry();
    registerModalBoxDriver({ boxDrivers: registry });
  ...
  test("registerModalBoxDriver with io registers a working factory closing over the transport", () => {
    const registry = new BoxDriverRegistry();
    registerModalBoxDriver({ boxDrivers: registry }, { transport: new FakeModalTransport() });

    const driver = registry.require("modal").create({}, makeDeps());
    assert.equal(driver.kind, "modal");
  });

  test("modalBoxDriverFactory constructs a working driver over the injected transport", async () => {
    const transport = new FakeModalTransport();
    const factory = modalBoxDriverFactory({ transport });
...
describe.skipIf(!LIVE_MODAL)("ModalBoxDriver (live, env-gated)", () => {
  test("provisions a real sandbox, probes it ready over ssh, and terminates it", async () => {
    ...
    // authored now and skipped by default. It would construct a ModalBoxDriver
